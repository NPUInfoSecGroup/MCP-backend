...
// Get nmap path from environment variable instead of argument
const nmapPath = process.env.NMAP_PATH;
if (!nmapPath) {
    console.error("NMAP_PATH environment variable not set");
    process.exit(1);
}
...
server.tool("do-nmap", "Run nmap with specified taget", {
    ...
    nmap_args: zod_1.z.union([
        zod_1.z.string().describe("Nmap arguments as a string (e.g. '-p 80,443 -sV')"),
        zod_1.z.array(zod_1.z.string()).describe("Nmap arguments as an array (e.g. ['-p', '80,443', '-sV'])")
...
MISC:
  ...
  -h: Print this help summary page.
    ...
    const parsedArgs = parseArgs(nmap_args);
    const nmap = (0, child_process_1.spawn)(nmapPath, [...parsedArgs, target]);
    let output = '';
    // Handle stdout
